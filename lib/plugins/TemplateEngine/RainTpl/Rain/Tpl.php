...
class Tpl 
{
    ...
    protected static $conf = array(
       ...
       'debug'           => false,
       'tpl_dir'         => 'templates/',
       'tpl_c_dir'       => 'rain/cache/php/',
       'tpl_ext'         => 'tpl',
       'cache_enable'    => false,
       'cache_dir'       => 'rain/cache/html/',
       'cache_time'      => 86400,
       'base_url'        => '',
       'php_enabled'     => false,
       ...
       'tags' => array(
           ...
           'switch'        => array('({switch.*?})','/{switch="([^"]*)"}/'),
           'switch_close'  => array('({\/switch})','/{\/switch}/'),
           'case'          => array('({case.*?})','/{case="([^"]*)"}/'),
           'case_close'    => array('({\/case})','/{\/case}/'),
    ...
    public static $templateInfo = Array();
    ...
    public function draw($templateFilePath, $toString = false) 
    {
        ...
        $template = $this->getTemplateCached($templateFilePath)!==false ? $this->getTemplateCached($templateFilePath) : $this->checkTemplate($templateFilePath);        
        ...
        $this->storeTemplateCached($templateFilePath,$html);
     ...
     * @return Int Nuber of file expired
     */
    public static function clean($expireTime = 2592000) 
    {
        $files  = glob(static::$conf['cache_dir'] . "*.rtpl.html");
 
        $time   = time();
        $totalFileExpired = 0;
        if(!$files){
            return $totalFileExpired;
        }
        foreach ($files as $file)
        {          
            if ($expireTime==-1 || (($time - filemtime($file) > $expireTime)))
            {
    ...
    /**
     * Returns path of cached Template
     * @param String $template template name
     * 
     * @return string|boolean
     */
    protected function getTemplateCached($template)
    {
        ...
        $cachedTemplateFilePath = static::$conf['cache_dir']   . $templateName . "." . md5($templateDirectory . serialize(static::$conf['checksum'])) . '.rtpl.html';
        
        //Check Cache
        if(!static::$conf['debug'] && static::$conf["cache_enable"] && file_exists($cachedTemplateFilePath) && filemtime($cachedTemplateFilePath) > filemtime($templateFilepath)){
           return $cachedTemplateFilePath;
        }
    ...
    protected function storeTemplateCached($template,$sourceHTML)
    {
        $templateName           = basename($template);
        $templateBasedir        = strpos($template, DIRECTORY_SEPARATOR) ? dirname($template) . DIRECTORY_SEPARATOR : null;
        $templateDirectory      = static::$conf['tpl_dir'] . $templateBasedir;
        ...
        if(!static::$conf['debug'] && static::$conf["cache_enable"])  //Check Cache
        {
           // create directories
           ...
           if(!file_exists($cachedTemplateFilePath) || (time() - filemtime($cachedTemplateFilePath)) > static::$conf['cache_time'])
           {
               $fp = fopen($cachedTemplateFilePath,"w");
               if(fwrite($fp,$sourceHTML)!==false){
                  if(fclose($fp)!==false){
                     return true;
                  }
               }
    ...
    protected function checkTemplate($template,$cache = false) 
    {
        // set filename
        $templateName           = basename($template);
        $templateBasedir        = strpos($template, DIRECTORY_SEPARATOR) ? dirname($template) . DIRECTORY_SEPARATOR : null;
        $templateDirectory      = static::$conf['tpl_dir'] . $templateBasedir;
        $templateFilepath       = $templateDirectory . $templateName . '.' . static::$conf['tpl_ext'];        
        $parsedTemplateFilepath = static::$conf['tpl_c_dir']   . $templateName . "." . md5($templateDirectory . serialize(static::$conf['checksum'])) . '.rtpl.php';
    ...
    protected function compileFile($templateName, $templateBasedir, $templateDirectory, $templateFilepath, $parsedTemplateFilepath) {
        ...
        if (flock($fp, LOCK_SH)) {
            ...
            self::$templateInfo['template_filepath'] = $templateFilepath;

            // read the file			
            self::$templateInfo['code'] = $code = fread($fp, filesize($templateFilepath));
            ...
            // create directories
            if (!is_dir(static::$conf['tpl_c_dir']))
                mkdir(static::$conf['tpl_c_dir'], 0755, TRUE);

            // check if the cache is writable
            if (!is_writable(static::$conf['tpl_c_dir']))
                throw new Tpl\Exception('Template  Directory ' . static::$conf['tpl_c_dir'] . 'doesn\'t have write permission. Set write permission or set RAINTPL_CHECK_TEMPLATE_UPDATE to FALSE. More details on http://www.raintpl.com/Documentation/Documentation-for-PHP-developers/Configuration/');
    ...
    protected function compileString($templateName, $templateBasedir, $templateFilepath, $parsedTemplateFilepath, $code) {
        ...
        if (flock($fp, LOCK_SH)) {
            ...
            // check if the cache is writable
            if (!is_writable(static::$conf['tpl_c_dir']))
                throw new Tpl\Exception('Cache directory ' . static::$conf['tpl_c_dir'] . 'doesn\'t have write permission. Set write permission or set RAINTPL_CHECK_TEMPLATE_UPDATE to false. More details on http://www.raintpl.com/Documentation/Documentation-for-PHP-developers/Configuration/');
        ...
        //read all parsed code
            foreach ($codeSplit as $html) {
                ...
                elseif (preg_match($tagMatch['include'], $html, $matches)) {
                    ...
                    //dynamic include
                    $parsedCode .= '<?php require_once $this->checkTemplate("' . $includeTemplate . '");?>';
                ...
                elseif (preg_match($tagMatch['loop'], $html, $matches)) {
                    ...
                    $parsedCode .= "<?php $counter=-1; $assignNewVar
                                    if(isset($$newvar) && ( is_array($$newvar) || $$newvar instanceof Traversable ) && sizeof($$newvar))
                                       foreach($$newvar as $key => $value ){ $counter++; ?>";
                }
                ...
                //switch tag
                elseif (preg_match($tagMatch['switch'], $html,$matches)) {
                    ...
                    // close if code
                    $parsedCode .= "<?php switch($parsedCondition){  ";
                }
                //close if tag
                elseif (preg_match($tagMatch['switch_close'], $html)) {

                    // close if code
                    $parsedCode .= ' } ?>';
                }
                //switch tag
                elseif (preg_match($tagMatch['case'], $html,$matches)) {
                    ...
                    // close if code
                    $parsedCode .= " case \"{$parsedCondition}\": ?>";
                }
                //close if tag
                elseif (preg_match($tagMatch['case_close'], $html)) {

                    // close if code
                    $parsedCode .= '<?php break; ';
                }
                 //close if tag
                elseif (preg_match($tagMatch['default'], $html)) {
    ...
    protected function varReplace($html, $loopLevel = NULL, $escape = TRUE, $echo = FALSE) {
        ...
        if (preg_match_all('/(\$[a-z_A-Z][^\s]*)/', $html, $matches)) {
            ...
            // update modifier
            $html = self::modifierReplace($html);
            ...
            if (!preg_match('/\$.*=.*/', $html)) {
                ...
                //$html = "htmlspecialchars( $html )";
                    $html = " $html ";// , ENT_COMPAT, '" . static::$conf['charset'] . "', FALSE )";
    ...
    public static function modifierReplace($html)
    {
        if ($pos = strrpos($html, "|")) 
        {
            // check black list
            ...
            $params    = isset($explode[1]) ? "," . implode(":",array_slice($explode,1,count($explode))) : null;
            
            
            if(method_exists(\Rain\Tpl\Modifiers::getClassName(),$function)){
               $html = "\Rain\Tpl\Modifiers::{$function}(" . self::modifierReplace(substr($html, 0, $pos)) . "$params)";
            }
            else
            {  
                $line = self::getTemplateCurrentLine($html);
                $e    = new Tpl\SyntaxException('Unknow Modifier::' . $function . ' not allowed in template: ' . self::$templateInfo['template_filepath'] . ' at line ' . $line);
                
                throw $e->templateFile(self::$templateInfo['template_filepath'])
                        ->tag($function)
                        ->templateLine($line);
            }
        }
    ...
    public static function getTemplateCurrentLine($html)
    {
       $line = 0;
       $rows = explode("\n", self::$templateInfo['code']);
    ...
    public static function blackList($html)
    {
        ...
        if (preg_match(self::$conf['black_list_preg'], $html, $match)) 
        {
            ...
            $e = new Tpl\SyntaxException('Syntax ' . $match[0] . ' not allowed in template: ' . self::$templateInfo['template_filepath'] . ' at line ' . $line);
            throw $e->templateFile(self::$templateInfo['template_filepath'])
                    ->tag($match[0])
                    ->templateLine($line);
